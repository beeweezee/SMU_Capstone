...
undesired_word_list = ['home','contact','web','us','usa','e','com','site','price','training','new','page','news','service','product','products','customer','u','business','linus','gracie','board','view','httpwww','zentocontemporary','e','p','r','l','h','nl','httpiowasteamjoker']
...
def remove_undesired_words(text):
    tokens = tokenizer.tokenize(text)
    tokens = [token.strip() for token in tokens]
    filtered_tokens = [token for token in tokens if token not in undesired_word_list]
    filtered_text = ' '.join(filtered_tokens)
    return filtered_text

def remove_undesirables(corpus):
    normalized_corpus = []
    for doc in corpus:
        doc = remove_undesired_words(doc)
